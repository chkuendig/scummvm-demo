const request = require('request'); // ctodo: replace with https://www.npmjs.com/package/axios
...
function parseTSV(text) {
    ...
    for (var i = 1; i < lines.length; i++) {
        ...
        delete ret[i-1][''] // there's an empty column in the sheet
    }
    return ret
}
...
function get_director_demos() {
    ...
    return new Promise((resolve, reject) => {
        ...
        getGoogleSheet(url, (error, response, body) => {
           ...
           return parseTSV(body).forEach((downloads) => {
...
// start everything
get_demos().then((demos) => {
    console.log(demos);
    // Write demos object to a file as JSON
    fs.writeFile('demos.json', JSON.stringify(demos, null, 2), (err) => {
        if (err) {
            console.error('Error writing file:', err);
        } else {
            console.log('Demos successfully written to demos.json');
        }
    });
}).catch((error) => {   
    console.error(error);
